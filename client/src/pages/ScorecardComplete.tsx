...
export default function ScorecardComplete() {
  ...
  useEffect(() => {
    if (!payload) {
      setState({
        status: "error",
        message: "We couldn't find your scorecard results. Try starting a free strategy session with Norman instead.",
      });
      return;
    }
    ...
    const createPlanner = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("We couldn't build your plan just yet. Try starting a free strategy session with Norman instead.");
        }

        const data = (await response.json()) as { editUrl?: string };
        if (!data.editUrl) {
          throw new Error("We couldn't load your planner link. Try starting a free strategy session with Norman instead.");
        }
        ...
        setState({
          status: "error",
          message: error instanceof Error ? error.message : "Something went wrong. Try starting a free strategy session with Norman instead.",
        });
      }
    };
